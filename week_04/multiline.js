...
d3.csv("long-term-interest-G7.csv").then(data => {
  ...
  let countries = new Set(); // array with only unique values
  ...
  let line = d3.line()  // define the generator outside the for-loop
    .x(d => x(d.Date))
    .y(d => y(d.Value));
  ...
  for (let country of countries) {
    ...
    let g = svg.append("g")
      .attr("class", "country")
      .on('mouseover', function () { // mouseover is the interative focus element 
        d3.selectAll(".highlight").classed("highlight", false);
        d3.select(this).classed("highlight", true);
      });

    if (country === "USA") {  // default highlight
