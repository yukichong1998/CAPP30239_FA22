...
d3.csv("../degrees.csv").then((data) => {
  ...
  const index = new Map(names.map((name, i) => [name, i])); // maps have definite positions
  ...
  const arc = d3.arc() // building the exterior labels
    .innerRadius(innerRadius)
    .outerRadius(outerRadius);

  // const ribbon = d3.ribbonArrow() // for directed chord diagrams
  ...
  svg.append("path") // building the inside path
    .attr("id", "text-id")
    .attr("fill", "none")
